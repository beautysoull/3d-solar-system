...
let moveThreshold = 5; // Порог минимального движения в пикселях для перемещения
let zoomSpeed = 0.02;  // Скорость изменения зума
let dragSpeed = 0.01;  // Скорость перемещения
...
canvas.addEventListener("touchmove", (event) => {
    ...
    if (event.touches.length === 1 && isDragging) {
        ...
        if (Math.abs(deltaX) > moveThreshold || Math.abs(deltaY) > moveThreshold) {
            cameraOffsetX += deltaX * dragSpeed;
            cameraOffsetY -= deltaY * dragSpeed;
            touchStartX = touch.clientX;
            touchStartY = touch.clientY;
        }
    }
